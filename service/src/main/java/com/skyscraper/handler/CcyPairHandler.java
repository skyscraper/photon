...
import com.photon.protocol.prototypes.gen.MarketPriceFlyweight;
import com.photon.protocol.prototypes.gen.OrderFlyweight;
import com.photon.protocol.prototypes.gen.ReconstructedBookFlyweight;
...
public class CcyPairHandler implements MessageHandler {
    ...
    private void sendBookUpdate(OrderBook orderBook, short venue, short side, CcyPair ccyPair)
    {
        ...
        final int size = workingOrderBook.getAggregatedOrderBookByLevel().size();
        ...
        if (index >= 0) {
            ...
            currentLevel.set(0);
            workingOrderBook.getAggregatedOrderBookByLevel().forEach((levelPrice, levelSize) -> {
